...
class ActiveRecordTreeBehavior extends CActiveRecordBehavior
{
	...
	private static $_tree_array = array();
	/**
	 ...
	 */
	public function getTree($addCriteria = array())
	{
		$cacheKey = ($this->cacheKey === null ? $this->owner->tableName() : $this->cacheKey);
	 ...
	 * @param string $cacheKey ключ для кэширования данных
	 */
	public function getTreeAsArray($addCriteria = array())
	{
		$cacheKey = ($this->cacheKey === null ? $this->owner->tableName() . 'array' : $this->cacheKey);
		if(isset(self::$_tree_array[$cacheKey]))
		{
			return self::$_tree_array[$cacheKey];
		}
		$tree = [];
		if(($cache = Yii::app()->cache) !== null && ($val = $cache->get($cacheKey)) !== false)
		{
		...
		else
		{
			...
			for($i = 0; $i < $countIntems; $i++)
			{
				$item = $items[$i];
				$id = $item->getPrimaryKey();
				$idParent = $item->$idParentField ? $item->$idParentField : 0;
		...
		self::$_tree_array[$cacheKey] = $tree;
		return $tree;
	}
	...
	{
		// чистим кеш
		Yii::app()->cache->delete($this->cacheKey === null ? $this->owner->tableName() : $this->cacheKey);
	}
}